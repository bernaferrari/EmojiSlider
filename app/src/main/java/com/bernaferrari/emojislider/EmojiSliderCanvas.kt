package com.bernaferrari.emojislider

import android.content.Context
import android.graphics.Canvas
import android.graphics.Color
import android.graphics.Typeface
import android.graphics.drawable.Drawable
import android.util.AttributeSet
import android.view.View
import android.widget.SeekBar
import com.github.florent37.mylittlecanvas.CanvasHelper
import com.github.florent37.mylittlecanvas.shape.RectShape
import com.github.florent37.mylittlecanvas.shape.TextShape
import com.github.florent37.mylittlecanvas.touch.EventPos
import com.github.florent37.mylittlecanvas.touch.ShapeEventManager
import com.github.florent37.mylittlecanvas.values.Alignment


class EmojiSliderCanvas @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0
) : View(context, attrs, defStyleAttr) {


    private val background = RectShape()
    private val indicator = MyDrawableShape()
    private val indicatorValue = TextShape()
    private val minText = TextShape()
    private val maxText = TextShape()

    private val shapeEventManager = ShapeEventManager(this)

    private fun init() {
        //configurations that does not depends on the view size
        background.setHeight(CanvasHelper.dpToPx(this, 15))
            .setCornerRadius(background.height / 2f)
            .setColor(Color.parseColor("#3F51B5"))
            .setVariable("margin", CanvasHelper.dpToPx(this, 20)).left =
                background.getVariable("margin")

        minText.setText("0")
            .setTextSizePx(CanvasHelper.dpToPx(this, 11))
            .setColor(Color.parseColor("#3E3E3E"))
            .setVerticalAlignment(Alignment.VERTICAL.BOTTOM)
            .setHorizontalAlignment(Alignment.HORIZONTAL.LEFT)
            .setLeft(background.left)
            .setTop(0f)
            .setVariable("distance", CanvasHelper.dpToPx(this, 5))

        maxText.setText("100")
            .setTextSizePx(CanvasHelper.dpToPx(this, 11))
            .setColor(Color.parseColor("#3E3E3E"))
            .setVerticalAlignment(Alignment.VERTICAL.BOTTOM)
            .setHorizontalAlignment(Alignment.HORIZONTAL.RIGHT)
            .setLeft(background.left).top = 0f

        indicator.apply {
            setDrawable(updateThumb("😍"))
//            minX = background.left
//            moveCenterXTo(-background.left)
        }

        indicatorValue.setText("0")
            .setTypeface(Typeface.DEFAULT_BOLD)
            .setTextSizePx(CanvasHelper.dpToPx(this, 11))
            .setColor(Color.parseColor("#3E3E3E"))
            .setVerticalAlignment(Alignment.VERTICAL.BOTTOM)
            .setHorizontalAlignment(Alignment.HORIZONTAL.CENTER)
            .setVariable("distance_with_bar", CanvasHelper.dpToPx(this, 8)).top = 0f

        shapeEventManager.onTouchAywhere { touchSetup ->
            touchSetup.move(indicator, RectShape.Pos.CENTER_X, EventPos.X)
//                .onDownAnimate { indicator.animate().radiusTo(background.height / 2) }
                .onDownAnimate {
                    indicatorValue.animate().bottomPlus(-1 * CanvasHelper.dpToPx(this, 10))
                }
                .onMove {

                    println("centerX: " + indicator.centerX + " bgWidth: " + background.width)

                    val percent =
                        ((indicator.centerX - background.left) / background.width * 100f).toInt()

                    indicatorValue.apply {
                        text = percent.toString()
                        left = indicator.centerX - 50
                        right = indicator.centerX + 50
                    }

                }
                .onUpAnimate {
                    indicatorValue
                        .animate()
                        .bottomTo(background.top - indicatorValue.getVariable<Float>("distance_with_bar"))
                }
//                .onUpAnimate {
//                    indicator
//                        .animate()
//                        .radiusTo(background.height)
//                        .setDuration(500)
//                }
        }
    }

    override fun onSizeChanged(width: Int, height: Int, oldw: Int, oldh: Int) {
        super.onSizeChanged(width, height, oldw, oldh)

        //configurations that depends on the view size

        background
            .setRight(width - background.getVariable<Float>("margin"))
            .centerVertical(height.toFloat())

        minText
            .setBottom(background.top - minText.getVariable<Float>("distance")).right =
                background.right

        maxText
            .setBottom(background.top - minText.getVariable<Float>("distance")).right =
                background.right

        indicator
            .setMinX(0f)
            .setMaxX(background.width)
            .centerVertical(height.toFloat())

        indicatorValue
            .setLeft(indicator.centerX - 50)
            .setRight(indicator.centerX + 50).bottom = background.top -
                indicatorValue.getVariable<Float>("distance_with_bar")

    }

    override fun onDraw(canvas: Canvas) {
        super.onDraw(canvas)
        background.onDraw(canvas)
        indicator.onDraw(canvas)
        indicatorValue.onDraw(canvas)
        minText.onDraw(canvas)
        maxText.onDraw(canvas)
    }

    var emojiHelper: EmojiHelper = EmojiHelper(this.context)
    val seekView = SliderDrawable(context)

    fun gradientColors(first: Int, second: Int) {
        seekView.sliderBar.color0_f32845l = first
        seekView.sliderBar.color1_f32846m = second
    }

    var sliderParticleSystem: View? = null
        set(value) {
            field = value

            if (value?.background !is EmojiHelper) {
                value?.background = emojiHelper
            }
        }

    init {
        initializer()
        secondView()
        init()
    }

    fun secondView() {
        seekView.callback = this
        seekView.f32871e = C5179d()
        seekView.m18503c()
        seekView.invalidateSelf()

//        slider2.background = seekView
    }

    fun initializer() {
//        sliderStickerSlider.setOnSeekBarChangeListener(this)
        updateThumb("😍")
    }

    fun onProgressChanged(seekBar: SeekBar, progress: Int, fromUser: Boolean) {
//
//        seekView.sliderBar.percentage_progress_f32847n = (progress / 100.0).toFloat()
////        seekView.updateShader()
//        seekView.invalidateSelf()
//
//        if (sliderParticleSystem == null) return
//
//        Logger.d("slider [top]: " + sliderStickerSlider.top + " // slider [paddingTop]: " + sliderStickerSlider.paddingTop + " // slider [bounds top]: " + sliderStickerSlider.thumb.bounds.top)
//
//        val sliderLocation = IntArray(2)
//        sliderStickerSlider.getLocationOnScreen(sliderLocation)
//
//        val particleLocation = IntArray(2)
//        sliderParticleSystem!!.getLocationOnScreen(particleLocation)
//
//        Logger.d("SLIDER - location [x]: " + sliderLocation[0] + " --- location [y]: " + sliderLocation[1])
//        Logger.d("PARTICLE - location [x]:" + particleLocation[0] + " --- location [y]: " + particleLocation[1])
//
//        if (fromUser) {
//            this.emojiHelper.onProgressChanged(
//                paddingLeft = sliderLocation[0].toFloat() + sliderStickerSlider.paddingLeft + sliderStickerSlider.thumb.bounds.left - particleLocation[0],
//                paddingTop = sliderLocation[1].toFloat() + Util.DpToPx(
//                    this.context,
//                    32f
//                ) - particleLocation[1]
//            )
//            this.emojiHelper.updateProgress(progress.toFloat() / 100.0f)
//        }
    }

    fun onStartTrackingTouch(seekBar: SeekBar) = emojiHelper.progressStarted()

    fun onStopTrackingTouch(seekBar: SeekBar) {
        emojiHelper.onStopTrackingTouch()
        seekView.sliderBar.cancelMethod()
    }

    private fun updateThumb(emoji: String): Drawable {

        emojiHelper.emoji = emoji

        return generateThumb(
            context = this.context,
            text = emoji,
            size = R.dimen.slider_sticker_slider_handle_size
        )
    }
}