...
class EmojiSlider @JvmOverloads constructor(
    ...
    init {
        ...
        try {
            ...
            val particleId = getParticleSystemView(array)
            if (particleId != 0) {
                println(
                    "findRecursiveResult is " + findParentRecursively(
                        this,
                        particleId
                    ) + " id: " + particleId
                )

                sliderParticleSystem = findParentRecursively(this, particleId)
            }
    ...
    private fun getParticleSystemView(typedArray: TypedArray): Int {
        return typedArray.getResourceId(R.styleable.EmojiSlider_particle_view_id, 0)
    }
    ...
    fun findParentRecursively(view: View, targetId: Int): View? {
        println("findRecursiveResult " + view.findViewById(targetId))
        if (view.findViewById<View?>(targetId) != null) {
            return view
        }
        println("findRecursiveResult parent: " + view.parent)
